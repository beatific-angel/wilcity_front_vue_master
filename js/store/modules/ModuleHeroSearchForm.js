...
const getters = {
  getSearchFields: state => postType => {
    ...
    if (!fields[postType]) {
      const rawFields = Vue.localStorage.get("hero_search_fields", false);
...
const mutations = {
  ...
  updateSearchFields(state, { fields, postType }) {
    ...
    let cacheFields = Vue.localStorage.get("hero_search_fields", false);
    ...
    Vue.localStorage.set(
      "hero_search_fields",
      JSON.stringify(state.searchFields)
    );
  }
};
