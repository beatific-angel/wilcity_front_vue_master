<template>
  <div>
    <wil-toggle-group
      v-for="(oGroup, index) in aGroupMenus"
      v-on:onDeleteItem="confirmRemoveItem(index)"
      :key="index"
      :is-auto-open="isAutoOpenGroup"
      :toggle-title="oGroup.group_title"
      wrapper-class="wil-group01_block__3X0D9"
    >
      <template v-slot:content>
        <div class="wil-group01_content__1oeej">
          <wiloke-input
            v-model="aGroupMenus[index].group_title"
            :settings="groupTitleSettings(oGroup)"
          ></wiloke-input>
          <wiloke-input
            v-model="aGroupMenus[index].group_description"
            :settings="groupDescriptionSettings(oGroup)"
          ></wiloke-input>
          <wiloke-icon
            v-model="aGroupMenus[index].group_icon"
            :settings="groupIconSettings(oGroup)"
          ></wiloke-icon>
        </div>
        <menu-list v-model="aGroupMenus[index].items" :a-values="getMenuList(oGroup)"></menu-list>
      </template>
    </wil-toggle-group>
    <a href="#" :class="addNewClass" @click.prevent="addGroupMenu">
      <i class="la la-plus"></i>
      <span v-text="''|filterTranslation('addNew')"></span>
    </a>
  </div>
</template>
<script>
    ...
    return {
      ...
      aDefaultGroup: {
        ...
        items: [
          {
            title:
              typeof WILCITY_I18.restaurant != "undefined"
                ? WILCITY_I18.restaurant.item.title
                : "My Title",
            description:
              typeof WILCITY_I18.restaurant != "undefined"
                ? WILCITY_I18.restaurant.item.description
                : "Price is net per person, excluding drinks chef seasonal menu",
            price:
              typeof WILCITY_I18.restaurant != "undefined"
                ? WILCITY_I18.restaurant.item.price
                : "$ 59.99 - $ 79.99",
            link_to: "",
            is_open_new_window: ""
          }
  ...
  computed: {
    maximumMenus() {
      return typeof this.$store.getters.getListingPlanSettings(
        "maximumRestaurantMenus"
      ) === "undefined"
        ? 0
        : parseInt(
            this.$store.getters.getListingPlanSettings(
              "maximumRestaurantMenus"
            ),
            10
          );
    ...
    maximumMenuWarning() {
      if (this.maximumMenus !== 0 && !isNaN(this.maximumMenus)) {
        return WILCITY_I18.maximumMenuWarning.replace("%s", this.maximumMenus);
      }
    },
    addNewClass() {
      return {
        ...
        disable:
          this.maximumMenus != 0 &&
          this.aGroupMenus.length === this.maximumMenus
      };
    }
  },
  methods: {
