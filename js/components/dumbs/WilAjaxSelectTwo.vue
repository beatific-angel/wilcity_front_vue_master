    ...
    <div class="field_wrap__Gv92k">
      <div v-if="!isAjax">
        <select v-if="settings.isMultiple=='yes'" v-model='selected' :class="selectTwoClass" multiple>
          <option v-for="option in aOptions" :value="printOptionValue(option)" v-html="printOptionName(option)"
                  :class="printOptionClass(option)"></option>
        </select>
        <select v-else v-model='selected' :class="selectTwoClass">
          <option v-for="option in aOptions" :value="printOptionValue(option)" :class="printOptionClass(option)"
                  v-html="printOptionName(option)"></option>
        </select>
      </div>
      <div v-else>
        <select v-if="settings.isMultiple=='yes'" v-model='selected' :class="selectTwoClass" multiple>
          <option selected v-for="option in aOptions" :value="printOptionValue(option)" v-html="printOptionName(option)"
                  :class="printOptionClass(option)"></option>
        </select>
        <select v-else v-model='selected' :class="selectTwoClass">
          <option selected v-for="option in aOptions" :value="printOptionValue(option)"
                  :class="printOptionClass(option)" v-html="printOptionName(option)"></option>
        </select>
      </div>
      <span class="field_label__2eCP7 text-ellipsis"
            :class="{required: settings.isRequired=='yes'}">{{settings.label}}</span>
    ...
    computed: {
      parseWrapperClasses(){
        ...
        if (!wrapperClasses.length) {
          wrapperClasses = 'field_module__1H6kT field_style2__2Znhe mb-15';
        }

        if (this.cId.length) {
          wrapperClasses += ' ' + this.cId;
        }
        if (this.isMultiple) {
          return this.selected.length ? wrapperClasses + ' active' : wrapperClasses;
        } else {
          return this.selected != null && this.selected != '' ? wrapperClasses + ' active' : wrapperClasses;
        }
      },
      isAjax(){
