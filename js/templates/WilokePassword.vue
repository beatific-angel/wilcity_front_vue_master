<template>
    <div :class="wrapperClass">
        <div class="field_wrap__Gv92k">
            <input v-if="isRequired(settings.isRequired)" v-model="value" class="field_field__3U_Rt" type="password" @keyup="changed">
            <input v-else v-model="value" class="field_field__3U_Rt" type="password" @keyup="changed">
            <span class="field_label__2eCP7 text-ellipsis" :class="{'required': isRequired(settings.isRequired)}">{{settings.label}}</span>
    ...
    export default{
        ...
        methods: {
            ...
            changed(){
                ...
                this.$emit('passwordChanged', this.value, this.settings);
            }
        }
    }
</script>
